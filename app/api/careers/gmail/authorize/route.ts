...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${CAREERS_API_URL}/api/v1/auth/gmail/authorize`, {
      headers: {
    ...
    if (!response.ok) return NextResponse.json({ error: data.detail || 'Failed to initiate Gmail OAuth' }, { status: response.status })
