...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${CAREERS_API_URL}/api/v1/recruiters/search`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) return NextResponse.json({ error: data.detail || 'Failed to search recruiters' }, { status: response.status })
